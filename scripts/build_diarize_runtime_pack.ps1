...
$packName = "meeting-minutes-diarize-runtime-$safeVersion-windows-x64.zip"
$packPath = Join-Path $outDirPath $packName
$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) "meeting-minutes-diarize-runtime-pack-$([guid]::NewGuid())"

try {
  New-Item -ItemType Directory -Force -Path $stagingRoot | Out-Null

  Copy-DirectoryTree $venvDir (Join-Path $stagingRoot ".venv-diarize")
  New-Item -ItemType Directory -Force -Path (Join-Path $stagingRoot "scripts") | Out-Null
  Copy-FileTo $backendScript (Join-Path $stagingRoot "scripts\diarize_cpu_backend.py")
  Copy-FileTo $requirements (Join-Path $stagingRoot "scripts\requirements-diarize-cpu.txt")
